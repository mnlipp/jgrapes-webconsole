...
Build tool is **jdbld** (JDrupes-Builder). The `build.gradle` at root is 
a **relic — do not run `./gradlew`**.

- Build config lives as **Java code** in `_jdbld/src/jdbld/`. Entry point: `Root.java`.
- Sub-projects are Java classes extending `AbstractProject`. 
  See `_jdbld/src/jdbld/console/`, `provider/`, `conlet/` for module definitions.
...
Modules that compile TypeScript: `org.jgrapes.webconsole.base`, 
`org.jgrapes.webconsole.vuejs`, `org.jgrapes.webconsole.provider.jgwcvuecomponents`,
`org.jgrapes.webconsole.provider.solidjs`, `org.jgrapes.webconsole.provider.chartjs`,
`aash-vue-components`, and several conlets (`locallogin`, `oidclogin`, `messagebox`,
`jmxbrowser`, `hellosolid`).
...
- Modules whose jdbld config class implements `Unpublishable` are not published
  to Maven.
- `WebConsoleExample` is excluded from `build` (use `./jdbld build` — it skips it
   automatically).
...
- Maven publishing targets **Codeberg** (`https://codeberg.org/api/packages/JGrapes/maven`),
  not Maven Central.
