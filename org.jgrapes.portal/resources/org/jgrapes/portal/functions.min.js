...
C(f,b);var n=!1;$(".column").each(function(b){if(b>=x.length)return!1;var c=x[b];if(B(c,a,a))return $(this).find(".portlet").each(function(b){b=$(this);var d=b.attr("data-portlet-id");if(!B(c,d,a))return b.before(f),n=!0,!1}),n||$(this).append(f),n=!0,!1});n||$(".column").first().prepend(f);p()}b=$(c);f.find(".portlet-header-text").text(b.attr("data-portlet-title"));c=f.find(".portlet-content");c.children().detach();c.append(b);d(b);e&&$("#portlet-tabs").tabs("option","active",0)}function p(){if(H){var a=
[];$(".overview-panel").find(".column").each(function(b){var f=[];a.push(f);$(this).find("div.portlet[data-portlet-id]").each(function(a){f.push($(this).attr("data-portlet-id"))})});var b=[],c=$("#portlet-tabs").tabs();c.find(".ui-tabs-nav .ui-tabs-tab").each(function(a){0<a&&(a=$(this).attr("aria-controls"),a=c.find("#"+a).attr("data-portlet-id"),b.push(a))});JGPortal.sendLayout(a,b)}}var k={debug:function(a){console&&console.debug&&console.debug(a)},info:function(a){console&&console.info&&console.info(a)},
warn:function(a){console&&console.warn&&console.warn(a)},error:function(a){console&&console.error&&console.error(a)}};JGPortal.log=k;JGPortal.formatMemorySize=function(a,b,c){void 0===c&&(c=b,b=-1);for(var f=0;1E4<a&&5>f;)a/=1024,f+=1;var d="PiB";switch(f){case 0:d="B";break;case 1:d="kiB";break;case 2:d="MiB";break;case 3:d="GiB";break;case 4:d="TiB"}return 0<=b?(new Intl.NumberFormat(c,{minimumFractionDigits:b,maximumFractionDigits:b})).format(a)+" "+d:(new Intl.NumberFormat(c)).format(a)+" "+d};
var l=function(){this._ws=null;this._sendQueue=[];this._recvQueue=[];this._recvQueueLocks=0;this._messageHandlers={};this._refreshTimer=null;this._inactivity=0;this._reconnectTimer=null;this._connectRequested=!1;this._initialConnect=!0;this._portalSessionId=this._connectionLostNotification=null;this._oldPortalSessionId=sessionStorage.getItem("org.jgrapes.portal.sessionId")};l.prototype.portalSessionId=function(){return this._portalSessionId};l.prototype._connect=function(){var a=("https:"===window.location.protocol?
"wss":"ws")+"://"+window.location.host+window.location.pathname;a.endsWith("/")||(a+="/");this._portalSessionId=sessionStorage.getItem("org.jgrapes.portal.sessionId");a+="portal-session/"+this._portalSessionId;this._oldPortalSessionId&&(a+="?was="+this._oldPortalSessionId,this._oldPortalSessionId=null);k.debug("Creating WebSocket for "+a);this._ws=new WebSocket(a);k.debug("Created WebSocket with readyState "+this._ws.readyState);var b=this;this._ws.onopen=function(){k.debug("OnOpen called for WebSocket.");
null!=b._connectionLostNotification&&(b._connectionLostNotification.notification("close"),$("#server-connection-restored-notification").notification({autoClose:2E3}));b._drainSendQueue();this._initialConnect?this._initialConnect=!1:(m().forEach(function(a){JGPortal.sendRenderPortlet(a,"Preview",!1)}),g().forEach(function(a){JGPortal.sendRenderPortlet(a,"View",!1)}));b._refreshTimer=setInterval(function(){0==b._sendQueue.length&&(b._inactivity+=y,0<z&&b._inactivity>=z?(b.close(),$("#portal-session-suspended-dialog").dialog("option",
"buttons",{Ok:function(){$(this).dialog("close");b.connect()}}),$("#portal-session-suspended-dialog").dialog("open")):b._send({jsonrpc:"2.0",method:"keepAlive",params:[]}))},y)};this._ws.onclose=function(a){k.debug("OnClose called for WebSocket (reconnect: "+b._connectRequested+").");null!==b._refreshTimer&&(clearInterval(b._refreshTimer),b._refreshTimer=null);b._connectRequested&&(null==b._connectionLostNotification?b._connectionLostNotification=$("#server-connection-lost-notification").notification({error:!0,
icon:"alert",destroyOnClose:!1}):b._connectionLostNotification.notification("open"),b._initiateReconnect())};this._ws.onmessage=function(a){try{var f=JSON.parse(a.data)}catch(u){k.error(u.name+":"+u.lineNumber+":"+u.columnNumber+": "+u.message+". Data: ");k.error(a.data);return}b._recvQueue.push(f);1===b._recvQueue.length&&b._handleMessages()}};l.prototype.connect=function(){this._connectRequested=!0;var a=this;$(window).on("beforeunload",function(){k.debug("Closing WebSocket due to page unload");
a._connectRequested=!1;a._ws.close()});this._connect()};l.prototype.close=function(){this._portalSessionId&&this._send({jsonrpc:"2.0",method:"disconnect",params:[this._portalSessionId]});this._connectRequested=!1;this._ws.close()};l.prototype._initiateReconnect=function(){if(!this._reconnectTimer){var a=this;this._reconnectTimer=setTimeout(function(){a._reconnectTimer=null;a._connect()},1E3)}};l.prototype._drainSendQueue=function(){for(;this._ws.readyState==this._ws.OPEN&&0<this._sendQueue.length;){var a=
this._sendQueue[0];try{this._ws.send(a),this._sendQueue.shift()}catch(n){k.warn(n)}}};l.prototype._send=function(a){this._sendQueue.push(JSON.stringify(a));this._drainSendQueue()};l.prototype.send=function(a){this._inactivity=0;this._send(a)};l.prototype.addMessageHandler=function(a,b){this._messageHandlers[a]=b};l.prototype._handleMessages=function(){for(;!(0===this._recvQueue.length||0<this._recvQueueLocks);){var a=this._recvQueue.shift(),b=this._messageHandlers[a.method];b?a.hasOwnProperty("params")?
b.apply(null,[].concat($jscomp.arrayFromIterable(a.params))):b():k.error("No handler for invoked method "+a.method)}};l.prototype.lockMessageReceiver=function(){this._recvQueueLocks+=1};l.prototype.unlockMessageReceiver=function(){--this._recvQueueLocks;0==this._recvQueueLocks&&this._handleMessages()};var h=new l;JGPortal.lockMessageQueue=function(){h.lockMessageReceiver()};JGPortal.unlockMessageQueue=function(){h.unlockMessageReceiver()};var y,z;JGPortal.init=function(a,b,c){sessionStorage.setItem("org.jgrapes.portal.sessionId",
a);y=b;z=c;h.connect();k.debug("Locking screen");$("body").faLoading({icon:"fa-circle-o-notch",spin:!0,text:!1});$("#theme-menu").on("click","[data-theme-id]",function(){JGPortal.sendSetTheme($(this).data("theme-id"));$("#theme-menu").jqDropdown("hide")});$("#language-menu").on("click","[data-locale]",function(){JGPortal.sendSetLocale($(this).data("locale"));$("#theme-menu").jqDropdown("hide")});$("#addon-menu").on("click","[data-portlet-type]",function(){JGPortal.sendAddPortlet($(this).data("portlet-type"));
$("#theme-menu").jqDropdown("hide")});$("#portlet-tabs").tabs();var f=$("#portlet-tabs").tabs();f.on("click","span.ui-icon-close",function(){var a=$(this).closest("li").remove().attr("aria-controls");$("#"+a).remove();f.tabs("refresh");p()});$("#portal-session-suspended-dialog").dialog({autoOpen:!1,modal:!0});$(".column").sortable({connectWith:".column",handle:".portlet-header",cancel:".portlet-toggle",placeholder:"portlet-placeholder ui-corner-all",stop:function(a,b){p()}});JGPortal.sendPortalReady()};
var F=new Set,r=[],t=new Set,G=!1;h.addMessageHandler("addPageResources",c);h.addMessageHandler("addPortletType",function(a,b,d,e,g){c(d,null,e);a=$('<li class="ui-menu-item"><div class="ui-menu-item-wrapper" data-portlet-type="'+a+'">'+b+"</div></li>");$("#addon-menu-list").append(a)});var x=[[],[],[]],H=!1,A={};h.addMessageHandler("lastPortalLayout",function(a,b){x=a;0<t.size&&(JGPortal.lockMessageQueue(),G=!0)});JGPortal.registerPortletMethod=function(a,b,c){var f=A[a];f||(f={},A[a]=f);f[b]=c};
h.addMessageHandler("notifyPortletView",function(a,b,c,d){(a=A[a])&&(c=a[c])&&c(b,d)});h.addMessageHandler("portalConfigured",function(){H=!0;p();k.debug("Unlocking screen");$("body").faLoading("remove")});JGPortal.findPortletPreview=function(a){a=$(".portlet[data-portlet-id='"+a+"']");if(1===a.length)return $(a[0])};var w=JGPortal.findPortletPreview;JGPortal.findPortletView=function(a){a=$("#portlet-tabs").tabs().find("> div[data-portlet-id='"+a+"']");if(1===a.length)return $(a[0])};var v=JGPortal.findPortletView;
JGPortal.updatePortletViewTitle=function(a,b){var c=$("#portlet-tabs").tabs();a=c.find("> div[data-portlet-id='"+a+"']");0!==a.length&&(a.find("[data-portlet-title]").attr("data-portlet-title",b),a=a.attr("id"),c=c.find("a[href='#"+a+"']"),c.empty(),c.append(b))};var L=$('<div class="portlet ui-widget ui-widget-content ui-helper-clearfix ui-corner-all"><div class="portlet-header ui-widget-header"><span class="portlet-header-text"></span></div><div class="portlet-content ui-widget-content"></div></div>');
JGPortal.updatePortletModes=function(a,b){(a=w(a))&&C(a,b)};var I=1;h.addMessageHandler("updatePortlet",function(a,b,c,e,g){if("Preview"===b||"DeleteablePreview"===b)D(a,c,e,g);else if("View"===b){c=v(a);e=$(e);if(c)c.children().detach(),c.append(e);else{b=$("#portlet-tabs").tabs();I+=1;c="portlet-tabs-"+I;var f=$("<li><a href='@{href}'>@{label}</a> <span class='ui-icon ui-icon-close' role='presentation'>Remove Tab</span></li>".replace(/@\{href\}/g,"#"+c).replace(/@\{label\}/g,e.attr("data-portlet-title")));
b.find(".ui-tabs-nav").append(f);c=$("<div id='"+c+"'></div>");c.append(e);c.attr("data-portlet-id",a);b.append(c);b.tabs("refresh");p()}d(e);g&&q(a)}else"Edit"===b&&(g=$(e),g.dialog({modal:!0,width:"auto"}),g.attr("data-portlet-id",a),d(g))});h.addMessageHandler("deletePortlet",function(a){var b=v(a);b&&(b=b.closest(".ui-tabs-panel").remove().attr("id"),$("#portlet-tabs").tabs().find("li[aria-controls='"+b+"']").remove(),$("#portlet-tabs").tabs().tabs("refresh"));(a=w(a))&&a.remove();p()});h.addMessageHandler("retrieveLocalData",
function(a){var b=[];try{for(var c=0;c<localStorage.length;c++){var d=localStorage.key(c);if(!a.endsWith("/")){if(d!==a)continue}else if(!d.startsWith(a))continue;var e=localStorage.getItem(d);b.push([d,e])}}catch(K){k.error(K)}JGPortal.sendLocalData(b)});h.addMessageHandler("storeLocalData",function(a){try{for(var b in a){var c=a[b];"u"===c[0]?localStorage.setItem(c[1],c[2]):"d"===c[0]&&localStorage.removeItem(c[1])}}catch(J){k.error(J)}});h.addMessageHandler("displayNotification",function(a,b){$(a).notification(b)});
h.addMessageHandler("reload",function(){window.location.reload(!0)});JGPortal.sendPortalReady=function(){h.send({jsonrpc:"2.0",method:"portalReady"})};JGPortal.sendSetTheme=function(a){h.send({jsonrpc:"2.0",method:"setTheme",params:[a]})};JGPortal.sendSetLocale=function(a){h.send({jsonrpc:"2.0",method:"setLocale",params:[a]})};JGPortal.sendRenderPortlet=function(a,b,c){h.send({jsonrpc:"2.0",method:"renderPortlet",params:[a,b,c]})};JGPortal.sendAddPortlet=function(a){h.send({jsonrpc:"2.0",method:"addPortlet",
params:[a,"Preview"]})};JGPortal.sendDeletePortlet=function(a){h.send({jsonrpc:"2.0",method:"deletePortlet",params:[a]})};JGPortal.notifyPortletModel=function(a,b,c){for(var d=[],e=2;e<arguments.length;++e)d[e-2]=arguments[e];void 0===d?h.send({jsonrpc:"2.0",method:"notifyPortletModel",params:[a,b]}):h.send({jsonrpc:"2.0",method:"notifyPortletModel",params:[a,b,d]})};JGPortal.sendLayout=function(a,b){h.send({jsonrpc:"2.0",method:"portalLayout",params:[a,b]})};JGPortal.sendLocalData=function(a){h.send({jsonrpc:"2.0",
method:"retrievedLocalData",params:[a]})}})();(function(){JGPortal.tooltip=function(a){a.tooltip({items:"[data-tooltip-id], [title]",content:function(){var a=$(this);if(a.is("[data-tooltip-id]")){a=a.attr("data-tooltip-id");var b=$("#"+a);return b?(b=b.clone(!0),b.removeClass("jgrapes-tooltip-prototype"),b):"<#"+a+">"}if(a.is("[title]"))return a.attr("title")}})}})();
